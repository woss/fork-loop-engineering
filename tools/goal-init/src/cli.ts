...
function parseArgs(argv: string[]) {
  ...
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    if (a === '--tool' || a === '-t') tool = argv[++i] as Tool;
    else if (a === '--dry-run') dryRun = true;
    else if (a === '--help' || a === '-h') return { help: true as const, tool, target, dryRun };
    else if (!a.startsWith('-')) target = a;
  }

  return { help: false as const, tool, target, dryRun };
}
...
async function resolveBundledOrMonorepo(name: 'templates'): Promise<string> {
...
async function runAuditJson(cli: string, targetDir: string): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    child.on('close', () => {
      ...
      else reject(new Error('goal-audit produced no output'));
    });
  });
}
...
`);
  ...
  const validTools = ['grok', 'claude', 'codex', 'opencode'] as Tool[];
